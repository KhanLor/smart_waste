...
// Get collectors with pagination and filters
$page = $_GET['page'] ?? 1;
$limit = 15;
$offset = ($page - 1) * $limit;
...
$sql = "
    SELECT u.*, 
           COUNT(cs.id) as assigned_schedules,
           COUNT(ch.id) as completed_collections
    FROM users u 
    LEFT JOIN collection_schedules cs ON u.id = cs.assigned_collector
    LEFT JOIN collection_history ch ON u.id = ch.collector_id AND ch.status = 'completed'
    WHERE {$where_clause}
    GROUP BY u.id
...
$stmt = $conn->prepare("
    ...
    FROM users u 
    JOIN collection_schedules cs ON u.id = cs.assigned_collector 
    WHERE u.role = 'collector' AND cs.status = 'active'
");
$stmt->execute();
...
$stmt = $conn->prepare("
    ...
    JOIN users u ON ch.collector_id = u.id 
    WHERE u.role = 'collector' AND DATE(ch.collection_date) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
");
$stmt->execute();
            ...
            <div class="col-md-3 col-lg-2 sidebar text-white p-0">
                <div class="p-3">
                    ...
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link text-white active" href="collectors.php">
            ...
            <div class="col-md-9 col-lg-10">
                <div class="p-4">
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card stat-card">
                                <div class="card-body text-center">
                                    ...
                                    <h4 class="mb-1"><?php echo $total_collectors_count; ?></h4>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card stat-card success">
                                <div class="card-body text-center">
                                    ...
                                    <h4 class="mb-1"><?php echo $active_collectors; ?></h4>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card stat-card warning">
                                <div class="card-body text-center">
                                    ...
                                    <h4 class="mb-1"><?php echo $total_collections; ?></h4>
                                    <small>Collections (30 days)</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card filter-card mb-4">
                        <div class="card-body">
                            <form method="GET" class="row g-3">
                                <div class="col-md-10">
                                    <input type="text" class="form-control" name="search" placeholder="Search collectors..." value="<?php echo e($search); ?>">
                                </div>
                                <div class="col-md-2">
                                    ...
                                    <div class="card collector-card">
                                        <div class="card-body">
                                            <div class="d-flex align-items-start mb-3">
                                                <div class="collector-avatar me-3">
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        ...
                                                        <li><a class="dropdown-item text-danger" href="#" onclick="deleteCollector(<?php echo $collector['id']; ?>)">
                                            ...
                                            <div class="mb-2">
                                                <small class="text-muted">
                                                    <i class="fas fa-envelope me-1"></i><?php echo e($collector['email']); ?>
                                                </small>
                                            </div>
                                            
                                            <?php if ($collector['phone']): ?>
                                                <div class="mb-2">
                                                    <small class="text-muted">
                                                        <i class="fas fa-phone me-1"></i><?php echo e($collector['phone']); ?>
                                                    </small>
                                                </div>
                                            <?php endif; ?>
                                            ...
                                            <div class="row text-center">
                                                <div class="col-6">
                                                    <div class="border-end">
                                                        <h6 class="mb-1 text-primary"><?php echo $collector['assigned_schedules']; ?></h6>
                                                        <small class="text-muted">Schedules</small>
                                                    </div>
                                                </div>
                                                <div class="col-6">
                                                    <h6 class="mb-1 text-success"><?php echo $collector['completed_collections']; ?></h6>
                                            ...
                                            <div class="mt-3">
                                                <small class="text-muted">
                                                    <i class="fas fa-calendar me-1"></i>Joined: <?php echo format_ph_date($collector['created_at'], 'M j, Y'); ?>
                                                </small>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <?php if ($total_pages > 1): ?>
                        <nav aria-label="Collectors pagination" class="mt-4">
    ...
    <div class="modal fade" id="editCollectorModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" id="editCollectorForm">
                    <div class="modal-body">
                        <input type="hidden" name="action" value="update_collector">
                        <input type="hidden" name="collector_id" id="editCollectorId">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this collector? This action cannot be undone.</p>
                    <p class="text-warning"><i class="fas fa-exclamation-triangle me-2"></i>Collectors with assigned schedules cannot be deleted.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <form method="POST" id="deleteForm" style="display: inline;">
                        <input type="hidden" name="action" value="delete_collector">
                        <input type="hidden" name="collector_id" id="deleteCollectorId">
